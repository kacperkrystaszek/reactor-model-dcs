    ...
    def init_phase(self):
        ...
        while len(self._clients) < len(self.EXPECTED_CLIENTS):
            try:
                ...
                ack_msg = json.dumps({"type": MessageType.ACK.value, "ack_for": MessageType.INIT.value, "config": self.config})
    ...
    def post_init_phase(self):
        ...
        while not all([True if val == States.WAIT_START else False for val in self._clients.values()]):
            try:
                ...
                if msg.get("type") != MessageType.ACK.value and msg.get("ack_for") != "CONFIG":
                    attempts += 1
                    continue
        ...
        for target_addr in self.EXPECTED_CLIENTS:
            ...
            while self._clients[target_addr] != States.RUNNING:
                ...
                try:
                    ...
                    if msg.get("type") == MessageType.ACK.value and msg.get("ack_for") == "START":
...
def apply_cpu_load(ip):
    ...
    proc = subprocess.Popen(
        ["ssh", f"pi@{ip}", f"sudo stress-ng --cpu {cores} --cpu-method matrixprod --oom-avoid --cpu-load 80 --timeout 370"],
        stdin=subprocess.DEVNULL,
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL
    )
...
def main(logger: Logger, rpi1Ip, rpi4Ip, scenario, rp1Load, rp4Load, beta, alpha, event_based):
