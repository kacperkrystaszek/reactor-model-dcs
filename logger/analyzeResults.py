...
def _plotOutputs(runs, title, nameBase):
    ...
    axY1.set_ylabel("Output Y1")
    axY2.set_title("Y2 + SP_Y2")
    axY2.set_ylabel("Output Y2")
    axEv1.set_title("Events Y1")
    axEv1.set_ylabel("Cumulative events Y1")
    axEv2.set_title("Events Y2")
    axEv2.set_ylabel("Cumulative events Y2")
    for axis in (axY1, axY2, axEv1, axEv2):
        axis.set_xlabel("Time [s]")
        axis.grid(True, alpha=0.3)
        axis.legend(loc="best", fontsize=8)

    fig.suptitle(f"{title} - Outputs & events", fontsize=13)
...
def buildLatexTable(scenario, load, records):
    ...
    header = (
        ...
        f"\\caption{{Quality indicators for scenario {caption}.}}\n"
        f"\\label{{{label}}}\n"
    ...
    footer = (
        ...
        "\\end{tabular}\n"
        "\\\\[2pt]\n"
        "\\footnotesize $N_e$ number of event invocations; IAE/IAU integrated over the run; "
        "$T_r$ settling time [s] (5\\% band); $M_p$ overshoot [\\%]. "
        "\"--\" means the channel does not take a step in that window.\n"
        "\\end{table}\n"
    )
    return header + "\n".join(lines) + footer
